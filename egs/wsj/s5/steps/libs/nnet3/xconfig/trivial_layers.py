...
class XconfigBlockSumLayer(XconfigLayerBase):
    ...
    def _generate_config(self):
        ...
        for ng_per_element_scale_opt_name in ['max-change', 'learning-rate-factor']:
            value = self.config[ng_per_element_scale_opt_name]
            if value != '':
                ng_per_element_scale_options += ' {0}={1}'.format(ng_per_element_scale_opt_name, value)
        ...
        configs = []
        line = ('component name={0}.element_wise_scale type=NaturalGradientPerElementScaleComponent '
                'dim={1} {2} {3} '.format(self.name, input_dim, opts, pes_str))
        configs.append(line)
